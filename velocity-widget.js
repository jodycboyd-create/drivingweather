/**
 * [weong-route] Module: Velocity Widget (Precision Sync)
 * Fix: Uses weighted averages for accurate NL travel times.
 */
(function() {
    let speedOffset = 0;
    let etaMarker = null;
    ...
    function initWidget() {
        ...
        ui.style.cssText = `position:absolute; bottom:30px; left:30px; z-index:9999; background:rgba(0,18,32,0.95); padding:20px; border-radius:12px; color:white; font-family:sans-serif; width:220px; border:1px solid #00B4DB; box-shadow: 0 10px 30px rgba(0,0,0,0.6); pointer-events: auto;`;
        ui.innerHTML = `
    ...
    function calculatePrecisionETA(route) {
        ...
        // [1] WEIGHTED SPEED CALCULATION
        route.instructions.forEach(instr => {
            const segDist = instr.distance / 1000;
            const roadName = (instr.road || "").toLowerCase();
            let segSpeed = 50; // Default local

            if (roadName.match(/trans-canada|nl-1|tch/)) {
                segSpeed = 100;
            } else if (roadName.match(/route|hwy|highway/)) {
                segSpeed = 80;
            } else if (segDist > 3) { 
                // Any long segment in NL is likely rural highway even if unnamed
                segSpeed = 80;
            }
            
            // Weight the speed based on how much of the trip it covers
        ...
        if (document.getElementById('totalDist')) {
            document.getElementById('totalDist').innerText = totalDist.toFixed(1);
        }

        // [2] FLAG ANCHOR: Use a permanent ID to prevent duplicate markers
        const midIdx = Math.floor(route.coordinates.length / 2);
        const midPoint = route.coordinates[midIdx];

        if (!etaMarker) {
            etaMarker = L.marker(midPoint, {
                pane: 'markerPane', // Force into the top-most HTML pane
                icon: L.divIcon({
                    className: 'eta-flag-container',
                    html: `<div id="flagTime" style="background:#1A73E8; color:white; padding:6px 12px; border-radius:20px; border:2px solid white; font-weight:bold; white-space:nowrap; box-shadow:0 4px 10px rgba(0,0,0,0.4); transform: translateY(-50%); font-family:sans-serif;">${timeStr}</div>`,
                    iconSize: [80, 40],
                    iconAnchor: [40, 20]
                })
