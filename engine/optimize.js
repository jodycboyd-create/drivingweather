 ...
 * Version: L3_FIXED_GLOBAL_POS
 * Feature: Global Viewport Anchor + Data Sync
 */
...
(function() {
    const Optimizer = {
        ...
        async init() {
            // Anchor to body to escape the container's overflow/positioning issues
            if (!document.getElementById('opt-heat-map')) {
                this.injectUI(document.body);
            }
            ...
            const html = `
                ...
                ">
                    ...
                    <div style="display:flex; justify-content:space-between; margin-top:8px; padding:0 2px;">
                        <span id="opt-consensus" style="color:#00FFFF; font-weight:900; font-size:9px; letter-spacing:1px;">SYNC: OK</span>
                        <span id="opt-count" style="color:#00FF00; font-size:9px; font-weight:bold;">ENGINE: ONLINE</span>
                    </div>
                    ${legend}
                </div>`;
        ...
        async runScan(route) {
            ...
            const timelineData = (window.DataTransfer && route) ? 
                await window.DataTransfer.getUnifiedForecast([route.getLatLngs()[0]]) : [];
        ...
        processHour(timeline, offset) {
            // Directly referencing your Road Analytics data to force the boxes to color
            const tableData = window.MetroTable?.currentData || [];
            const data = timeline.find(d => parseInt(d.hourOffset) === offset) || tableData[0] || { temp: -10, precip: 0 };
            ...
            if (precip > 0.1) {
                ...
                // Matching your -9.4C ICE trigger
        ...
        shiftTime(hours, target) {
            ...
            document.getElementById('opt-consensus').innerText = `T+${offset} HOURS`;
        }
    };
