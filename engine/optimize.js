 ...
 * Mission: Hazard Origin Reporting + Strict Icon Gating
 */
...
(function() {
    const Optimizer = {
        ...
        // Milestone names for tracking
        milestones: ["START", "WEST COAST", "CENTRAL", "EASTERN", "DESTINATION"],
            ...
            const html = `
                <div id="opt-heat-map" style="margin-bottom:15px; border-bottom:1px solid #FFD700; padding-bottom:10px; font-family:monospace;">
                    ...
                    <div style="display:flex; justify-content:space-between; margin-top:8px;">
                        <span id="opt-origin" style="color:#FFD700; font-weight:900; font-size:9px; letter-spacing:1px;">SCANNING ROUTE...</span>
                        <span id="opt-status" style="color:#00FF00; font-size:9px;">PRECIP LOCK</span>
                    </div>
                </div>`;
        ...
        async runScan(route) {
            ...
            for (let i = 0; i < 24; i++) {
                ...
                if (i === 0) { // Report origin for current selected window
                    document.getElementById('opt-origin').innerText = res.level > 1 ? `HAZARD: ${res.location}` : "ROUTE CLEAR";
                }
            }
        },
        ...
        async checkPrecip(points, offset) {
            ...
            let maxL = 0, isSnow = false, worstLoc = "CLEAR";
            try {
                ...
                res.forEach((d, index) => {
                    ...
                    let L = 0;
                    if (code === 65 || code === 75 || code >= 95) L = 4;
                    else if (code === 63 || code === 73) L = 3; 
                    else if (code === 61 || code === 71) L = 2;
                    else if (code >= 51 && code <= 55) L = 1;

                    if (L > maxL) { 
                        maxL = L; 
                        worstLoc = this.milestones[index];
                        isSnow = (code >= 71 && code <= 75) || code >= 85; 
                    }
                });
            } catch (e) { return {level: 0, isSnow: false, location: "ERROR"}; }
            return {level: maxL, isSnow: isSnow, location: worstLoc};
        },
